...
class SegList(torch.utils.data.Dataset):
    def __init__(self, args, data_dir, phase, transforms, image_dir=None, gt_dir=None, targets=None, list_dir=None, out_name=False, pred=[], prob=[]):
        ...
        if self.phase == 'labeled':
            self.pred_idx = torch.nonzero(pred.int(), as_tuple=False)
            self.probability = [prob[i] for i in self.pred_idx]

        elif self.phase == 'unlabeled':
            self.pred_idx = torch.nonzero(1-pred.int(), as_tuple=False)
...
class Trackometer(object):
    ...
    def update(self, sal_pred, GT_label, sal_pred_list, pseudolabels):
        ...
        for dummy_ind in range(len(pseudolabels)):
            Loss_temp += F_cont(sal_pred_list[dummy_ind].cuda(), pseudolabels[dummy_ind], b=1.0)
