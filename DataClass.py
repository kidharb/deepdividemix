...
class BatchData():
    ...
    def compute_saliency_custom_input(self, model, DCRF, inputs):
        out = model(inputs)
        self.out_all = out
        self.output = out[0]
        self.features = out[2]
    ...
    def merge_pseudolabels(self):
        self.merged_labels = torch.div(torch.sum(torch.stack(self.pseudolabels_var),dim=0),len(self.pseudolabels_var))
    ...
    #Compute
    def compute_loss(self, mean_loss=True, beta=1.0):
        self.loss=0.0
        for dummy_ind in range(len(self.sal_pred_list)):
            self.loss+=F_cont(self.sal_pred_list[dummy_ind], self.pseudolabels_var[dummy_ind], mean_loss, b=beta)
